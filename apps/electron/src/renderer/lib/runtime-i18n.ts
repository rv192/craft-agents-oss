...
const RUNTIME_LITERAL_OVERRIDES: Record<string, string> = {
  ...
  'Follow System': '跟随系统',
  'PowerShell: List directory contents (dir/ls equivalent)': 'PowerShell：列出目录内容（dir/ls 等价）',
  'PowerShell: Read file contents (cat equivalent)': 'PowerShell：读取文件内容（cat 等价）',
...
const POWERSHELL_COMMENT_SUFFIX_OVERRIDES: Record<string, string> = {
  'List directory contents (dir/ls equivalent)': '列出目录内容（dir/ls 等价）',
  'Read file contents (cat equivalent)': '读取文件内容（cat 等价）',
  'Get file/directory item properties': '获取文件/目录项属性',
  'Get item properties': '获取项属性',
  'Print current directory (pwd equivalent)': '打印当前目录（pwd 等价）',
  'List running processes': '列出运行中的进程',
  'List services': '列出服务',
  'List available commands': '列出可用命令',
  'Display command help': '显示命令帮助',
  'List loaded modules': '列出已加载模块',
  'List variables': '列出变量',
  'List command aliases': '列出命令别名',
  'Get host information': '获取主机信息',
  'Get current date/time': '获取当前日期/时间',
  'Get culture settings': '获取区域设置',
  'Get system information': '获取系统信息',
  'List drives': '列出驱动器',
  'Get command history': '获取命令历史',
  'Read event logs': '读取事件日志',
  'Read Windows events': '读取 Windows 事件',
  'Get access control list': '获取访问控制列表',
  'List network adapters': '列出网络适配器',
  'Get IP addresses': '获取 IP 地址',
  'Get routing table': '获取路由表',
  'Get DNS server addresses': '获取 DNS 服务器地址',
  'List disks': '列出磁盘',
  'List volumes': '列出卷',
  'List disk partitions': '列出磁盘分区',
  'List installed packages': '列出已安装包',
  'List installed PowerShell modules': '列出已安装的 PowerShell 模块',
  'Test if path exists': '测试路径是否存在',
  'Ping equivalent': 'Ping 等价命令',
  'Test network connectivity': '测试网络连通性',
  'Resolve path to full path': '将路径解析为完整路径',
  'Split path into components': '将路径拆分为各部分',
  'Join path components': '拼接路径组件',
  'Measure/count objects': '测量/计数对象',
  'Measure execution time': '测量执行时间',
  'Compare two objects': '比较两个对象',
  'Select object properties': '选择对象属性',
  'Sort objects': '排序对象',
  'Group objects by property': '按属性分组对象',
  'Filter objects': '筛选对象',
  'Iterate over objects': '遍历对象',
  'Format output as list': '将输出格式化为列表',
  'Format output as table': '将输出格式化为表格',
  'Format output as wide table': '将输出格式化为宽表',
  'Convert to JSON': '转换为 JSON',
  'Parse JSON': '解析 JSON',
  'Convert to CSV': '转换为 CSV',
  'Parse CSV': '解析 CSV',
  'Convert to XML': '转换为 XML',
  'Search strings in files (grep equivalent)': '在文件中搜索字符串（grep 等价）',
  'Search for modules in gallery': '在库中搜索模块',
  'Search for packages': '搜索包',
  'Version information variable': '版本信息变量',
  'Environment variable access': '环境变量访问',
  'Current directory variable': '当前目录变量',
  'Home directory variable': '主目录变量',
  'Host information variable': '主机信息变量',
  'Get-ChildItem alias': 'Get-ChildItem 别名',
  'Get-Content alias': 'Get-Content 别名',
  'Get-Process alias': 'Get-Process 别名',
  'Get-Service alias': 'Get-Service 别名',
  'Get-Command alias': 'Get-Command 别名',
}
...
export function applyRuntimeTranslationOverrides(map: TranslationMap): TranslationMap {
  ...
  for (const [suffixSource, suffixTarget] of Object.entries(POWERSHELL_COMMENT_SUFFIX_OVERRIDES)) {
    next[normalizeText(`PowerShell: ${suffixSource}`)] = `PowerShell：${suffixTarget}`
  }
